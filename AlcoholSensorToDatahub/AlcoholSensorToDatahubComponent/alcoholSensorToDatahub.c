...
)
{
	...
	res= ma_alcoholSensor_Read(&alcohol);

	if (res!= LE_OK) {
		LE_INFO("FAILED to read alcohol sensor");
	}

	LE_INFO("alcohol = %f ug/m3\n",alcohol);

	io_PushNumeric(ALCOHOL_NAME, IO_NOW, alcohol);
...
COMPONENT_INIT
{
	...
	result = io_CreateInput(ALCOHOL_NAME, IO_DATA_TYPE_NUMERIC, "degrees");
